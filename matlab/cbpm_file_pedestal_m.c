		 ...
		 const mxArray * prhs[]) 
{
  ...
  ncols = mxGetN(prhs[2]);
  if (!(mrows == 1 && ncols == 1)) {
      mexErrMsgTxt("Third argument must be a noncomplex scalar integer.");   /* returns to Matlab */
  }
  button = (int)mxGetScalar(prhs[2]);


  // Call target library function.
  retval = cbpm_rawfile_pedestal( inst_name, &bunch, &button );
